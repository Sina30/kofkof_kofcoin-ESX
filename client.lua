...
RegisterNetEvent('kofkof:getkofcoinSTART')
AddEventHandler('kofkof:getkofcoinSTART', function()
...
Citizen.CreateThread(function()
    while SERVER_LOADED == false do
        TriggerServerEvent('kofkof:givekofcoinSTART', GetSourceId())
        Citizen.Wait(10)
    end
end)
        ...
        for _, Machine in pairs(Machines) do
            for i, Server in pairs(Machine.machines) do
                ...
                if dist <= 2.0 then
                    ...
                    Drawing.draw3DText(Server.x, Server.y, Server.z, "[~g~Server "..Server.id.."~w~ - "..health.."] ~n~ Press [~g~G~w~] to ~r~remove~w~", 4, 0.10, 0.06, 255, 255, 255, 255)
    ...
    while true do
        ...
        if Machines ~= nil and SERVER_LOADED == true then
            for i, Machine in pairs(Machines) do
                ...
                if dist <= 2.0 then
                    Drawing.draw3DText(monitor.x, monitor.y, monitor.z-0.85, "[~g~Monitor~w~]~n~ Press [~g~E~w~] to ~g~open~w~~n~Press [~g~H~w~] to ~r~remove~w~", 4, 0.07, 0.03, 255, 255, 255, 255)
            ...
            for _, Machine in pairs(Machines) do
                for i, Server in pairs(Machine.machines) do
                    if Server.status == true then
                        ...
                        Server.kofcoins = tonumber(string.format("%.5f", (Server.kofcoins + (Config.CoinsPerUpdate * Server.GPUlevel))))
                    ...
                    elseif Server.status == false then
                        ...
                        if Server.temperature > 0 then
                            local LessTemprature = tonumber(string.format("%.2f", (Config.TemperaturePerUpdate*Server.fans)))
                            Server.temperature = tonumber(string.format("%.2f", (Server.temperature - LessTemprature)))
                        end
                    end
                end
            end
...
RegisterCommand("sellcoins", function(source, args, rawCommand)
    if args[1] ~= nil then
        ...
        Notification("~r~Invalid Coin Amount~w~")
    end
end)
