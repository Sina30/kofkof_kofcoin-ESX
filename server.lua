RegisterNetEvent('kofkof:givekofcoinSTART')

AddEventHandler('kofkof:givekofcoinSTART', function(source)

	TriggerClientEvent('kofkof:getkofcoinSTART', source)
...
AddEventHandler('kofkof:GetMachines', function(source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM kofmachines WHERE dono = @dono", {['@dono'] = GetLicensse(source)}, function(Machines)  

        if Machines[1] == nil then

            MySQL.Async.execute("INSERT INTO kofmachines (dono) VALUE (@dono)",

            {

                ['@dono']   = GetLicensse(source),
    ...
    MySQL.Async.fetchAll("SELECT machine FROM kofmachines", {}, function(Machines)  
...
AddEventHandler('kofkof:CreateMachine', function(source, machine)
    ...
    MySQL.Async.execute("UPDATE kofmachines SET machine = @machine WHERE dono = @dono",
...
AddEventHandler('kofkof:DeleteMachine', function(source)
    ...
    MySQL.Async.execute("DELETE FROM kofmachines WHERE dono = @dono",
...
AddEventHandler('kofkof:addcoinsdb', function(source, total)
    ...
    MySQL.Async.execute("UPDATE kofmachines SET coins = (coins + @coins) WHERE dono = @dono",
...
AddEventHandler('kofkof:sellcoins', function(source, amount)
    ...
    MySQL.Async.fetchAll("SELECT * FROM kofmachines WHERE dono = @dono", {['@dono'] = GetLicensse(source)}, function(Machines)  
        ...
        if amount <= coins then
            ...
            MySQL.Async.execute("UPDATE kofmachines SET coins = @coins WHERE dono = @dono",
